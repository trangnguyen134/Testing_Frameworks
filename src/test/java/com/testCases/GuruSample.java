...
import com.utilities.Guru_Util;
...
public class GuruSample {
	...
	public Object[][] testData() {
		...
		// 1st row
		data[0][0] = Guru_Util.USER_NAME;
		data[0][1] = Guru_Util.PASSWD;
		//2nd row
		data[1][0] = "invalid";
		data[1][1] = "valid";
		//3rd row
	...
	public void setUp() throws Exception {
		/*
		 * File pathToBinary = new File(Util.FIREFOX_PATH); FirefoxBinary ffBinary = new
		 * FirefoxBinary(pathToBinary); FirefoxProfile firefoxProfile = new
		 * FirefoxProfile(); driver = new FirefoxDriver(ffBinary, firefoxProfile);
		 */
		WebDriverManager.chromedriver().setup();
		driver = new ChromeDriver();

		// Setting Base URL of website Guru99
		baseUrl = Guru_Util.BASE_URL;
		driver.manage().timeouts()
				.implicitlyWait(Guru_Util.WAIT_TIME, TimeUnit.SECONDS);
	...
	public void testCase05(String username, String password) throws Exception {
		...
		driver.findElement(By.name("btnLogin")).click();
		 ...
		 try{ 
				...
				assertEquals(actualBoxMsg,Guru_Util.EXPECT_ERROR);
				...
				String[] parts = pageText.split(Guru_Util.PATTERN);
				String dynamicText = parts[1];

				// Check that the dynamic text is of pattern mngrXXXX
				// First 4 characters must be "mngr"
				assertTrue(dynamicText.substring(1, 5).equals(Guru_Util.FIRST_PATTERN));
				...
				assertTrue(remain.matches(Guru_Util.SECOND_PATTERN));
				// Code to take Screenshot
				File scrFile = ((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE);
				// Code to save screenshot at desired location
				FileUtils.copyFile(scrFile, new File("D:\\Ammie\\Study\\Selenium_Cert\\EclipseRepo\\Testing_Frameworks\\screenshots\\screenshot.png"));
	        } 
		}		
